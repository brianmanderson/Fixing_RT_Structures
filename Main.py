...
def convert_rts(base_path):
    for path, directory, files in os.walk(base_path):  # Look through all folders
        files = [i for i in files if i.endswith('.dcm')]  # Stop if we have something with dicom
        if files and len(files) > 5:  # Only go forward if we have at least 5 images
...
if __name__ == '__main__':
    convert_rts(base_path=r'K:\Morfeus\YHe\Brian')
